...
const wallet = new ethers.Wallet('510aadec6e9a46fe0ef154f990922cc28fbfdab7ebe62352e927c80be3c33ce71d');  // private key
...
let getpool = async () => {
    ...
    for (let a of token) {
        ...
        fs.appendFile(path.join('poolsWBNB.txt'), `'${a}': {
                `, 'utf8', (err) => {
        ...
        fs.appendFile(path.join('poolsWBNB.txt'), `
}`, 'utf8', (err) => {
                if (err) console.log(`problem writing pair`)})  
    }
...
let result = async (a,b) => {
    ...
    try {
        ...
        const pairAddress = await factoryContract.getPair(WBNB,a);
        const contract = new web3.eth.Contract(abi,a);
        const symbol = await contract.methods.symbol().call()
