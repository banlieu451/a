...
async function buy() {
        ...
        console.log("\u001b[1;32m" + "âœ” Buy transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\u001b[0m");
...
try {
	...
	const receipt = await tx.wait();
	console.log("âœ” Approve transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\n");
	if (config.autoSell) {
...
async function sell(tokenObj, isProfit) {
	...
	try {
		...
		if (tokenObj.tokenSellTax > 1) {
			...
			const receipt = await tx.wait();
			console.log("\u001b[1;32m" + "âœ” Sell transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\u001b[0m", "\n");
			sellCount++;
