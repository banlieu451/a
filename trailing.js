...
async function buy() {
        ...
        console.log("\u001b[1;32m" + "Ã¢Å“â€ Buy transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\u001b[0m");
...
try {
	...
	const receipt = await tx.wait();
	console.log("Ã¢Å“â€ Approve transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\n");
	if (config.autoSell) {
...
async function sell(tokenObj, isProfit) {
    const gasLimit_ = 125000 * tokenObj.tokenSellTax
    const gasLimit = gasLimit_.toString();
	try {
		...
		if (tokenObj.tokenSellTax > 1) {
			...
			const receipt = await tx.wait();
			console.log("\u001b[1;32m" + "Ã¢Å“â€ Sell transaction hash: ", `https://bscscan.com/tx/${receipt.transactionHash}`, "\u001b[0m", "\n");
			sellCount++;
			...
			const tx = await SolidityContractPancakeSwap.swapExactTokensForETH(
				sellAmount[0].toString(),
				0,
				tokenObj.sellPath,
				addresses.recipient,
