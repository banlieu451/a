...
async function buy() {
	...
	if (buyCount < config.numberOfTokensToBuy && isScam == false) {
		const value = ethers.utils.parseUnits('0.001', 'ether').toString();
		...
		console.log("\u001b[1;32m" + "âœ” Buy transaction hash: ", receipt.transactionHash, "\u001b[0m");
...
async function approve() {
	...
	let contract = token[buyCount].tokenAddress;
	console.log(contract);

	try {
    
		const ApproveContracttoDECODED = new ethers.Contract
		(
			contract, //  1/3
		  [
		  ...
		  const fulltoken_ = '0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'
		  const ApproveTx = await ApproveContracttoDECODED.approve(
			addresses.pancakeRouter,
			fulltoken_, 
		  {
			  "gasLimit": web3.utils.toHex(95000),// try aligne to buy from SMART C : 95000
			  "gasPrice": web3.utils.toHex(5000000000),
			  "nonce": nonce
		  })
		  await ApproveTx;
		  console.log(`<<<<<--- Approve ${token} on ${contractAddress} ---->>>>>`)
		  console.log(("Hash.         ") + ("https://bscscan.com/tx/")+(ApproveTx.hash));
		  if (config.autoSell) {
...
//  		);
//  		const receipt = await tx.wait();
//  		console.log("âœ” Approve transaction hash: ", receipt.transactionHash, "\n");
//  		if (config.autoSell) {
//  			token[buyCount - 1].checkProfit();
//  		} else {
//  			if (buyCount == config.numberOfTokensToBuy) {
...
async function sell(tokenObj, isProfit) {
	try {
		...
		if (tokenObj.tokenSellTax > 1) {
			...
			const receipt = await tx.wait();
			console.log("\u001b[1;32m" + "âœ” Sell transaction hash: ", receipt.transactionHash, "\u001b[0m", "\n");
			sellCount++;
